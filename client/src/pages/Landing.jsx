...
export default function Landing() {
  ...
  return (
    <div style={{ minHeight:'100vh', background:'#060a10', color:'#f0f4ff', fontFamily:"'DM Sans',sans-serif", overflowX:'hidden' }}>
      ...
      {/* BG orbs — animated drifting */}
      <div style={{ position:'fixed', inset:0, pointerEvents:'none', zIndex:0 }}>
        <motion.div
          animate={{ x:[0,30,-10,-30,0], y:[0,-20,20,-10,0], scale:[1,1.05,0.95,1.02,1] }}
          transition={{ duration:20, repeat:Infinity, ease:'linear' }}
          style={{ position:'absolute', top:'-10%', left: isMobile ? '0' : '20%', width: isMobile ? 300 : 600, height: isMobile ? 300 : 600, borderRadius:'50%', background:'radial-gradient(circle, rgba(99,102,241,0.12) 0%, transparent 70%)', filter:'blur(40px)' }}
        />
        <motion.div
          animate={{ x:[0,-25,20,15,0], y:[0,15,-25,20,0], scale:[1,0.97,1.04,0.98,1] }}
          transition={{ duration:25, repeat:Infinity, ease:'linear' }}
          style={{ position:'absolute', bottom:'5%', right:'-5%', width: isMobile ? 200 : 400, height: isMobile ? 200 : 400, borderRadius:'50%', background:'radial-gradient(circle, rgba(20,184,166,0.09) 0%, transparent 70%)', filter:'blur(40px)' }}
        />
        <motion.div
          animate={{ x:[0,15,-20,10,0], y:[0,-15,10,-20,0] }}
          transition={{ duration:18, repeat:Infinity, ease:'linear' }}
          style={{ position:'absolute', top:'40%', left:'60%', width: isMobile ? 150 : 300, height: isMobile ? 150 : 300, borderRadius:'50%', background:'radial-gradient(circle, rgba(236,72,153,0.06) 0%, transparent 70%)', filter:'blur(40px)' }}
        />
      </div>
      ...
      <div style={{ position:'relative', zIndex:1, paddingTop: isMobile ? 100 : 140, paddingBottom:80, maxWidth:1000, margin:'0 auto', padding: isMobile ? '100px 20px 60px' : '140px 40px 80px', textAlign:'center' }}>
        <motion.div initial={{ opacity:0, y:28 }} animate={{ opacity:1, y:0 }} transition={{ duration:0.6 }}>
          ...
          <motion.div
            ...
            style={{ display:'inline-flex', alignItems:'center', gap:6, padding:'5px 14px', borderRadius:100, marginBottom:24, background:'rgba(99,102,241,0.1)', border:'1px solid rgba(99,102,241,0.25)', fontSize:11, fontWeight:700, color:'#818cf8', textTransform:'uppercase', letterSpacing:'0.08em' }}
          >
            <motion.span
          ...
          <motion.div
            ...
            style={{ fontFamily:"'Syne',sans-serif", fontWeight:800, fontSize: isMobile ? '42px' : 'clamp(48px,7vw,82px)', lineHeight:1.05, letterSpacing:'-0.03em', marginBottom:8, color:'#f0f4ff' }}
          >
            Work{' '}
            <span style={{ display:'inline-block', minWidth: isMobile ? 180 : 260, verticalAlign:'bottom' }}>
              <AnimatePresence mode="wait">
                <motion.span
                  key={wordIdx}
                  initial={{ opacity:0, y:18, rotateX:-30, filter:'blur(4px)' }}
                  animate={{ opacity:1, y:0, rotateX:0, filter:'blur(0px)' }}
                  exit={{ opacity:0, y:-18, rotateX:30, filter:'blur(4px)' }}
          ...
          <motion.div
            ...
            style={{ fontFamily:"'Syne',sans-serif", fontWeight:800, fontSize: isMobile ? '42px' : 'clamp(48px,7vw,82px)', lineHeight:1.05, letterSpacing:'-0.03em', marginBottom:24, color:'#f0f4ff' }}
          >
            with AI
          </motion.div>
          ...
          <motion.p
            ...
            style={{ fontSize: isMobile ? 15 : 18, color:'#6b7db3', maxWidth:520, margin:'0 auto 36px', lineHeight:1.75 }}
          >
          ...
          >
            <motion.button onClick={() => navigate('/register')} whileHover={{ scale:1.04, boxShadow:'0 8px 40px rgba(99,102,241,0.5)' }} whileTap={{ scale:0.97 }}
            ...
            <motion.button onClick={() => navigate('/login')} whileHover={{ scale:1.02, borderColor:'rgba(255,255,255,0.25)' }}
          ...
          <motion.div
            ...
            style={{ display:'flex', alignItems:'center', justifyContent:'center', gap: isMobile ? 16 : 24, flexWrap:'wrap', color:'#4b5563', fontSize:13 }}
          >
            {['Free forever','No credit card','Deploy in minutes'].map((text, i) => (
              <motion.div
                key={text}
        ...
        >
          <motion.div
            whileHover={{ y:-4, boxShadow:'0 50px 120px rgba(0,0,0,0.7), 0 0 80px rgba(99,102,241,0.12)' }}
            transition={{ duration:0.4 }}
            style={{ background:'#0b1120', border:'1px solid rgba(255,255,255,0.08)', borderRadius:20, padding: isMobile ? 14 : 24, boxShadow:'0 40px 100px rgba(0,0,0,0.7), 0 0 60px rgba(99,102,241,0.08)', overflowX:'auto' }}
          >
            {/* Window dots */}
            ...
            <div style={{ display:'grid', gridTemplateColumns: isMobile ? '1fr 1fr' : 'repeat(4,1fr)', gap: isMobile ? 8 : 12, minWidth: isMobile ? 'auto' : 0 }}>
              {(isMobile ? MOCK_COLS.slice(0,2) : MOCK_COLS).map((col, colIdx) => (
                <motion.div
                  key={col.label}
                  initial={{ opacity:0, y:15 }}
                  animate={{ opacity:1, y:0 }}
                  transition={{ delay:0.8 + colIdx * 0.1 }}
                  style={{ background:'rgba(255,255,255,0.02)', borderRadius:10, padding:10 }}
                >
                  ...
                  {col.tasks.slice(0, isMobile ? 2 : 3).map((t,i) => (
                    <motion.div
                      ...
                      transition={{ delay:1.0 + colIdx * 0.1 + i * 0.08 }}
                      style={{ background:'#0e1526', borderRadius:6, padding:'7px 9px', marginBottom:5, border:'1px solid rgba(255,255,255,0.05)', borderLeft:`2px solid ${col.color}` }}
                    >
          ...
          <motion.div
            ...
            style={{ position:'absolute', bottom:-36, left:'10%', right:'10%', height:70, background:'rgba(99,102,241,0.18)', filter:'blur(36px)', borderRadius:'50%', zIndex:-1 }}
          />
        </motion.div>
      </div>
        ...
        <div style={{ display:'grid', gridTemplateColumns: isMobile ? '1fr 1fr' : 'repeat(3,1fr)', gap: isMobile ? 10 : 16 }}>
          {features.map(({ icon:Icon, title, desc, color }, i) => (
            <motion.div key={title}
              ...
              whileHover={{ y:-6, borderColor:`${color}40`, boxShadow:`0 20px 40px rgba(0,0,0,0.3), 0 0 20px ${color}15` }}
              style={{ padding: isMobile ? '16px 14px' : '24px 22px', borderRadius:14, background:'#0b1120', border:'1px solid rgba(255,255,255,0.06)', transition:'border-color 0.3s, box-shadow 0.3s' }}
            >
              <motion.div
                whileHover={{ scale:1.1, rotate:5 }}
                transition={{ type:'spring', stiffness:400 }}
                style={{ width: isMobile ? 38 : 44, height: isMobile ? 38 : 44, borderRadius:11, marginBottom:12, background:`${color}18`, border:`1px solid ${color}30`, display:'flex', alignItems:'center', justifyContent:'center' }}
              >
      ...
      <div style={{ position:'relative', zIndex:1, maxWidth:620, margin:'0 auto', padding: isMobile ? '20px 20px 80px' : '40px 40px 100px', textAlign:'center' }}>
        <motion.div
          ...
          style={{ padding: isMobile ? '36px 24px' : '48px 40px', borderRadius:22, background:'#0b1120', border:'1px solid rgba(99,102,241,0.3)', boxShadow:'0 0 60px rgba(99,102,241,0.08)', position:'relative', overflow:'hidden' }}
        >
          {/* Animated glow behind CTA */}
          <motion.div
            animate={{ opacity:[0.3,0.6,0.3], scale:[1,1.1,1] }}
            transition={{ duration:4, repeat:Infinity, ease:'easeInOut' }}
            style={{ position:'absolute', top:'-50%', left:'20%', width:'60%', height:'200%', background:'radial-gradient(circle, rgba(99,102,241,0.08) 0%, transparent 70%)', pointerEvents:'none' }}
          />
          <motion.div
          ...
          <h2 style={{ fontFamily:"'Syne',sans-serif", fontSize: isMobile ? 26 : 32, fontWeight:800, marginBottom:12, color:'#f0f4ff', letterSpacing:'-0.02em', position:'relative' }}>
            Ready to level up?
          </h2>
          <p style={{ color:'#6b7db3', marginBottom:28, fontSize: isMobile ? 14 : 15, lineHeight:1.7, position:'relative' }}>
          ...
          <motion.button onClick={() => navigate('/register')}
            ...
            whileTap={{ scale:0.97 }}
            style={{ padding: isMobile ? '13px 32px' : '14px 44px', fontSize: isMobile ? 15 : 16, fontWeight:600, fontFamily:'inherit', borderRadius:12, background:'linear-gradient(135deg,#6366f1,#4f46e5)', border:'none', color:'white', cursor:'pointer', display:'inline-flex', alignItems:'center', gap:8, boxShadow:'0 4px 24px rgba(99,102,241,0.4)', position:'relative' }}>
      ...
      <motion.div
        ...
        style={{ borderTop:'1px solid rgba(255,255,255,0.05)', padding: isMobile ? '20px 20px' : '24px 40px', textAlign:'center', color:'#374151', fontSize:12 }}
      >
